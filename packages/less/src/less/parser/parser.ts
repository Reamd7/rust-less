...
const Parser = function Parser(context, imports, fileInfo, currentIndex = 0) {
    const parsers = {
        //
        ...
        //
        primary: () => {
            const mixin = parsers.mixin;
            let root = [];
            let node;

            while (true) {
                while (true) {
                    node = parsers.comment();
                    if (!node) { break; }
                    root.push(node);
                }
                ...
                node = parsers.extendRule();
                if (node) {
                    root = root.concat(node);
                    continue;
                }

                node = mixin.definition() || parsers.declaration() || mixin.call(false, false) ||
                    parsers.ruleset() || parsers.variableCall() || parsers.entities.call() || parsers.atrule();
                if (node) {
                    root.push(node);
                } else {
        ...
        entities: {
            ...
            call: function () {
                ...
                name = name[1];
                func = parsers.entities.customFuncCall(name);
                if (func) {
                    args = func.parse();
                    if (args && func.stop) {
                        parserInput.forget();
                ...
                args = parsers.entities.arguments(args);
            ...
            arguments: function (prevArgs) {
                ...
                while (true) {
                    if (prevArgs) {
                        ...
                        value = parsers.detachedRuleset() || parsers.entities.assignment() || parsers.expression();
                        if (!value) {
                            break;
                        }
            ...
            literal: function () {
                return  parsers.entities.dimension() ||
                        parsers.entities.color() ||
                        parsers.entities.quoted() ||
                        parsers.entities.unicodeDescriptor();
            },
            ...
            url: function () {
                ...
                value = parsers.entities.quoted() || parsers.entities.variable() || parsers.entities.property() ||
                        parserInput.$re(/^(?:(?:\\[()'"])|[^()'"])+/) || '';
        ...
        variableCall: function (parsedName) {
            ...
            if (name || (parserInput.currentChar() === '@'
                ...
                lookups = parsers.mixin.ruleLookups();
        ...
        mixin: {
            ...
            call: function (inValue, getLookup) {
                ...
                elements = parsers.mixin.elements();

                if (elements) {
                    if (parserInput.$char('(')) {
                        args = parsers.mixin.args(true).args;
                        expectChar(')');
                        hasParens = true;
                    }
            ...
            args: function (isCall) {
                ...
                while (true) {
                    if (isCall) {
                        ...
                        arg = entities.variable() || entities.property() || entities.literal() || entities.keyword() || parsers.mixin.call(true);
                    }
            ...
            definition: function () {
                ...
                if (match) {
                    ...
                    const argInfo = parsers.mixin.args(false);
                    params = argInfo.args;
            ...
            ruleLookups: function() {
                ...
                while (true) {
                    parserInput.save();
                    rule = parsers.mixin.lookupValue();
                    if (!rule && rule !== '') {
                        parserInput.restore();
                        break;
                    }
        ...
        entity: function () {
            ...
            return parsers.comment() || entities.literal() || entities.variable() || entities.url() ||
                entities.property() || entities.call() || entities.keyword() || parsers.mixin.call(true) ||
                entities.javascript();
        },
        ...
        element: function () {
            ...
            c = parsers.combinator();

            e = parserInput.$re(/^(?:\d+\.\d+|\d+)%/) ||
                // eslint-disable-next-line no-control-regex
                parserInput.$re(/^(?:[.#]?|:*)(?:[\w-]|[^\x00-\x9f]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+/) ||
                parserInput.$char('*') || parserInput.$char('&') || parsers.attribute() ||
                parserInput.$re(/^\([^&()@]+\)/) ||  parserInput.$re(/^[.#:](?=@)/) ||
                parsers.entities.variableCurly();

            if (!e) {
                parserInput.save();
                if (parserInput.$char('(')) {
                    if ((v = parsers.selector(false)) && parserInput.$char(')')) {
                        e = new(tree.Paren)(v);
                        parserInput.forget();
                    } else {
                        parserInput.restore('Missing closing \')\'');
        ...
        selector: function (isLess) {
            ...
            let condition;
            isLess = isLess !== false;
            while ((isLess && (extendList = parsers.extend())) || (isLess && (when = parserInput.$str('when'))) || (e = parsers.element())) {
                if (when) {
                    condition = expect(parsers.conditions, 'expected condition');
                } else if (condition) {
        ...
        block: function () {
            let content;
            if (parserInput.$char('{') && (content = parsers.primary()) && parserInput.$char('}')) {
                return content;
            }
        },

        blockRuleset: function() {
            let block = parsers.block();
        ...
        detachedRuleset: function() {
            ...
            if (parserInput.$re(/^[.#]\(/)) {
                 ...
                 */
                argInfo = parsers.mixin.args(false);
                params = argInfo.args;
                variadic = argInfo.variadic;
                if (!parserInput.$char(')')) {
                    parserInput.restore();
            ...
            const blockRuleset = parsers.blockRuleset();
            if (blockRuleset) {
                parserInput.forget();
                if (params) {
        ...
        ruleset: function () {
            ...
            selectors = parsers.selectors();

            if (selectors && (rules = parsers.block())) {
                parserInput.forget();
        ...
        declaration: function () {
            ...
            name = parsers.variable() || parsers.ruleProperty();
            if (name) {
                ...
                if (!value) {
                    ...
                    if (name[0].value && name[0].value.slice(0, 2) === '--') {
                        value = parsers.permissiveValue(/[;}]/);
                    }
                    // Try to store values as anonymous
                    // If we need the value later we'll re-parse it in ruleset.parseValue
                    else {
                        value = parsers.anonymousValue();
                    }
                    if (value) {
                        parserInput.forget();
        ...
        'import': function () {
            ...
            if (dir) {
                const options = (dir ? parsers.importOptions() : null) || {};

                if ((path = parsers.entities.quoted() || parsers.entities.url())) {
                    features = parsers.mediaFeatures({});
            ...
            do {
                ...
                if (e) {
                    ...
                    if (!p && syntaxOptions.queryInParens && parserInput.$re(/^[0-9a-z-]*\s*([<>]=|<=|>=|[<>]|=)/)) {
                        ...
                        parserInput.save();
                        rangeP = parsers.atomicCondition(null, p.rvalue);
                        if (!rangeP) {
                            parserInput.restore();
                        }
                    } else {
        ...
        mediaFeatures: function (syntaxOptions) {
            ...
            let e;
            do {
                e = parsers.mediaFeature(syntaxOptions);
                if (e) {
                    features.push(e);
                    if (!parserInput.$char(',')) { break; }
                } else {
        ...
        prepareAndGetNestableAtRule: function (treeType, index, debugInfo, syntaxOptions) {
            const features = parsers.mediaFeatures(syntaxOptions);

            const rules = parsers.block();
        ...
        nestableAtRule: function () {
            ...
            if (parserInput.$peekChar('@')) {
                if (parserInput.$str('@media')) {
                    return parsers.prepareAndGetNestableAtRule(tree.Media, index, debugInfo, MediaSyntaxOptions);
                }
                
                if (parserInput.$str('@container')) {
                    return parsers.prepareAndGetNestableAtRule(tree.Container, index, debugInfo, ContainerSyntaxOptions);
                }
            }
        ...
        atrule: function () {
            ...
            value = parsers['import']() || parsers.plugin() || parsers.nestableAtRule();
            if (value) {
                return value;
            }
        ...
        conditions: function () {
            ...
            a = parsers.condition(true);
            if (a) {
                while (true) {
                    if (!parserInput.peek(/^,\s*(not\s*)?\(/) || !parserInput.$char(',')) {
                        break;
                    }
                    b = parsers.condition(true);
                    if (!b) {
                        break;
                    }
        ...
        condition: function (needsParens) {
            ...
            result = parsers.conditionAnd(needsParens);
            if (!result) {
                return ;
            }
            logical = or();
            if (logical) {
                next = parsers.condition(needsParens);
                if (next) {
        ...
        conditionAnd: function (needsParens) {
            ...
            logical = and();
            if (logical) {
                next = parsers.conditionAnd(needsParens);
                if (next) {
                    result = new(tree.Condition)(logical, result, next);
                } else {
                    return ;
        ...
        negatedCondition: function (needsParens) {
            if (parserInput.$str('not')) {
                const result = parsers.parenthesisCondition(needsParens);
                if (result) {
        ...
        parenthesisCondition: function (needsParens) {
            ...
            body = tryConditionFollowedByParenthesis(parsers);
            if (body) {
                parserInput.forget();
                return body;
            }

            body = parsers.atomicCondition(needsParens);
            if (!body) {
                parserInput.restore();
                return ;
            }
        ...
        atomicCondition: function (needsParens, preparsedCond) {
            ...
            const cond = (function() {
                return parsers.addition() || entities.keyword() || entities.quoted() || entities.mixinLookup();
            }).bind(this)
        ...
        operand: function () {
            ...
            let o = parsers.sub() || entities.dimension() ||
        ...
        expression: function () {
            ...
            do {
                ...
                e = parsers.addition() || parsers.entity();
    ...
    function error(msg: string, type?: string) {
        throw new LessError(
            {
                index: parserInput.i,
    ...
    const ParserIns = {
        ...
        parse: function (str, callback, additionalData) {
            ...
            try {
                ...
                root = new tree.Ruleset(null, parsers.primary());
