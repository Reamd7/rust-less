...
export default () => {
    let input: string; // Less input string
    ...
    let furthestPossibleErrorMessage: string | undefined;
    ...
    const parserInput = {
        ...
        isWhitespace: (offset?: number) => {
            const pos = parserInput.i + (offset || 0);
            const code = input.charCodeAt(pos);
            return (code === CHARCODE_SPACE || code === CHARCODE_CR || code === CHARCODE_TAB || code === CHARCODE_LF);
        },
        restore: (possibleErrorMessage?: string) => {
         ...
         */
        $parseUntil: (tok: string | RegExp) => {
            let quote: string | string[] | null | undefined = '';
            let returnVal = null;
            let inComment = false;
            let blockDepth = 0;
            const blockStack = [];
            ...
            let i = parserInput.i;
            let loop = true;
            let testChar: (char: string) => boolean;
        ...
        commentStore: [] as { index: number; text: string; isLineComment: boolean; }[],
        finished: false,

        // Same as $(), but don't change the state of the parser,
        // just return the match.
        peek: (tok: string | RegExp) => {
            if (typeof tok === 'string') {
                // https://jsperf.com/string-startswith/21
        ...
        peekChar: (tok: string) => input.charAt(parserInput.i) === tok,

        currentChar: () => input.charAt(parserInput.i),
        prevChar: () => input.charAt(parserInput.i - 1),
        getInput: () => input,
        ...
        start: (str: string, chunkInput: boolean | undefined, failFunction: (msg: string, index: number) => void) => {
            input = str;
    ...
    function skipWhitespace(length: number) {
        ...
        for (; parserInput.i < endIndex; parserInput.i++) {
            ...
            if (parserInput.autoCommentAbsorb && c === CHARCODE_FORWARD_SLASH) {
                ...
                if (nextChar === '/') {
                    const commentIndex = parserInput.i
                    ...
                    comment = {
                        index: commentIndex, 
                        text: inp.substr(commentIndex, parserInput.i - commentIndex), 
                        isLineComment: true
                    };
